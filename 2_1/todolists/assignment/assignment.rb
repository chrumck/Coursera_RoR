...
class Assignment
  ...
  def create_todolist(params)
    # accept a hash of todolist properties (`:name` and `:due_date`) as an input parameter. Note these are not 100% the
    # same as Model class. use the TodoList Model class to create a new user in the DB
    # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
    new_list = TodoList.new
    params.each { |key, value| new_list.send("list_#{key}=", value) }
    new_list.save
  end
  ...
  def find_allusers(offset, limit)
    ...
    # use the User Model class to find all Users, ordered by `updated_at` ascending, with specified row offset and row
    # limit. Return a collection of User instances that represent the specified rows
    User.order(:updated_at).offset(offset).limit(limit).to_a
  end

  def find_alllists(offset, limit)
    # accept offset and limit input parameters
    # use the TodoList Model class to find all TodoLists, ordered by `list_due_date` descending, with specified row
    # offset and row limit. Return a collection of TodoList instances that represent the specified rows
